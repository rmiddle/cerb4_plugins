...
function drawChart(start, end) {{/literal}
	...
	var cObj = YAHOO.util.Connect.asyncRequest('GET', "{/literal}{devblocks_url}ajax.php?c=reports&a=action&extid=report.group.answernet.client.time&extid_a=getTimeSpentClientChart{/devblocks_url}{literal}&countonly=1&worker_id="+worker+"&group_id="+group+"&start="+start+"&end="+end, {
		success: function(o) {
			...
			var myXHRDataSource = new YAHOO.util.DataSource("{/literal}{devblocks_url}ajax.php?c=reports&a=action&extid=report.group.answernet.client.time&extid_a=getTimeSpentClientChart{/devblocks_url}{literal}&worker_id="+worker+"&group_id="+group+"&start="+start+"&end="+end);
...
<h2>{$translate->_('reports.ui.group.answernet.client.time')}</h2>
