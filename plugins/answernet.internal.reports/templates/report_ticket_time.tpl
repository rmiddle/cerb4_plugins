...
function drawChart(start, end) {{/literal}
	...
	var cObj = YAHOO.util.Connect.asyncRequest('GET', "{/literal}{devblocks_url}ajax.php?c=reports&a=action&extid=report.group.answernet.ticket.time&extid_a=getTimeSpentTicketChart{/devblocks_url}{literal}&countonly=1&worker_id="+worker+"&group_id="+group+"&start="+start+"&end="+end, {
		success: function(o) {
			...
			var myXHRDataSource = new YAHOO.util.DataSource("{/literal}{devblocks_url}ajax.php?c=reports&a=action&extid=report.group.answernet.ticket.time&extid_a=getTimeSpentTicketChart{/devblocks_url}{literal}&worker_id="+worker+"&group_id="+group+"&start="+start+"&end="+end);
...
<h2>{$translate->_('reports.ui.group.answernet.ticket.time')}</h2>

<form action="{devblocks_url}{/devblocks_url}" method="POST" id="frmRange" name="frmRange" onsubmit="return false;">
<input type="hidden" name="c" value="reports">
<input type="hidden" name="a" value="action">
<input type="hidden" name="extid" value="report.group.answernet.ticket.time">
